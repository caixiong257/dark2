...
#include "D2DataTables.h"	//BINÎÄ¼þ
...
typedef int(__fastcall *D2ItemSpell_t)(DWORD, UnitAny*, UnitAny*, UnitAny*, int, int, int);
...
struct D2ItemSpellTableStrc {
	//Used by identify pSpell to change cursor
	D2ItemSpell_t InitFunc;
	//Address of the function to call
	D2ItemSpell_t GameFunc;
};
...
struct PresetItem  //D2ItemDrop size 0x84
{
	UnitAny* pOwner;		//0x00
	DWORD dwSeed;			//0x04
	DWORD pGame;			//0x08
	DWORD iLvl;				//0x0C
	DWORD _2;				//0x10 Unknown
	DWORD iIdx;				//0x14
	DWORD iMode;			//0x18
	DWORD xPos;				//0x1C
	DWORD yPos;				//0x20
	...
	DWORD nDropFlags;				//0x80 Some flags can be 2 | 8, a2 | a3 Flag 1 -> More magic items - thx Nefarius
};
...
struct ClientData //size 0x518
{
	...
	BYTE PlayerStatus;			   //0x0A 0x4 connected, 0x8 death, 0x20 expansion char
	BYTE CompleteActFlags;		   //0x0B
	BYTE ClassId2;				   //0x0C
	...
	DWORD _4;		               //0x64
	DWORD pGameData;			   //0x68 (seems to be added by d2gs, not by original blizz game) 
	...
	DWORD ActNo;					   //0x1AC
	DWORD _8;	                   //0x1B0
	DRLGRoom* ptRoom;				   //0x1B4
	DWORD Packet[3];		   //0x1B8
	DWORD _10[132];				   //0x1C4
	DWORD dwFlags;				   //0x3D4
...
struct PlayerData { //size 0x16C
	char szName[16];				//0x00
	QuestFlags* pQuestsFlags[3];	//0x10
	Waypoint * pWaypoints[3];		//0x1C
...
struct DRLGRoom {
	DRLGRoom** pRoomsNear; 		//0x00 pptVisibleRooms
	DWORD _1;					//0x04
	void *_1s;					//0x08
	DWORD _1b;					//0x0C
	DRLGRoomEx* pRoomEx;		//0x10
	DWORD _2[2];				//0x14
...
struct Path {
	...
	WORD yTarget;					//0x12
	DWORD _2[2];					//0x14
	DRLGRoom *pRoom1;					//0x1C
	DRLGRoom *pRoomUnk;				//0x20
	DWORD _3[3];					//0x24
	UnitAny *pUnit;					//0x30
	DWORD dwFlags;					//0x34
	DWORD _4;						//0x38
...
struct Skill {
	SkillsTxt *pSkillTxt;			//0x00
	Skill *pNextSkill;				//0x04
	DWORD nSkillMode;				//0x08
	DWORD dwType;					//0x0C
	DWORD _1[6];					//0x10
...
struct ObjectData {
	...
	DWORD pShrineRecord;				// +08
	DWORD dwOpGUID;						// +0C
	BOOL  bPermanent;					// +10
	DWORD nUnk;							// +14
	DRLGCoord pDestRoom;					// +18
	DRLGCoord pDestPortal;				// +20
	char szPerant[16];					// +28
};

struct ObjectPath {
	DRLGRoom *pRoom1;					//0x00
	DWORD _1[2];					//0x04
	DWORD dwPosX;					//0x0C
	DWORD dwPosY;					//0x10
