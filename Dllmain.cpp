...
{
	MessageBoxA(NULL, szMessage, "D2Plugin Error", MB_OK | MB_ICONERROR);
...
BOOL __fastcall D2TEMPLATE_ApplyPatch(void* hGame, const DLLPatchStrc* hPatch)
{
	...
	while (hPatch->nDLL != D2DLL_INVALID)
	{
		...
		DWORD dwAddress = hPatch->dwAddress;//偏移量地址
		if (!dwAddress) return FALSE;

		DWORD dwBaseAddress = gptDllFiles[nDLL].dwAddress;//获取相应DLL的基址
		if (!dwBaseAddress) return FALSE;

		dwAddress += dwBaseAddress;//真实的内存地址

		DWORD dwData = hPatch->dwData;
		//如果是相对位置则重新计算补丁内容
		if (hPatch->bRelative) { dwData = dwData - (dwAddress + sizeof(dwData) + hPatch->bRelative); }

		void* hAddress = (void*)(dwAddress + hPatch->bRelative);//地址转换为指针
		DWORD dwOldPage;//旧内存保护方式

		//如果规定了内存补丁大小
		if (hPatch->nPatchSize > 0)
		{
			BYTE Buffer[1024];//申请缓冲区
			...
			VirtualProtect(hAddress, hPatch->nPatchSize, dwOldPage, &dwOldPage);
		}
		else
		{
